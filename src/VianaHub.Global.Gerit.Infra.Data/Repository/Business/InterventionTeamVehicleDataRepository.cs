...
public class InterventionTeamVehicleDataRepository : IInterventionTeamVehicleDataRepository
{
    private readonly GeritDbContext _context;

    public InterventionTeamVehicleDataRepository(GeritDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<ListPage<InterventionTeamVehicleEntity>> GetPagedAsync(PagedFilter request, CancellationToken ct)
    {
        ...
        return new ListPage<InterventionTeamVehicleEntity>
        {
            Data = result,
    ...
    {
        await _context.Set<InterventionTeamVehicleEntity>().AddAsync(entity, ct);
        return await _context.SaveChangesAsync(ct) > 0;
    }

    public async Task<bool> UpdateAsync(InterventionTeamVehicleEntity entity, CancellationToken ct)
    {
        _context.Set<InterventionTeamVehicleEntity>().Update(entity);
        return await _context.SaveChangesAsync(ct) > 0;
    }
}
