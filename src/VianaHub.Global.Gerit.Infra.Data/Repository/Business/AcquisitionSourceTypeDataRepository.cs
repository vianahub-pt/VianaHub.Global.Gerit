...
public class AcquisitionSourceTypeDataRepository : IAcquisitionSourceTypeDataRepository
{
    private readonly GeritDbContext _context;

    public AcquisitionSourceTypeDataRepository(GeritDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<ListPage<AcquisitionSourceTypeEntity>> GetPagedAsync(PagedFilter request, CancellationToken ct)
    {
        ...
        return new ListPage<AcquisitionSourceTypeEntity>
        {
            Items = result,
    ...
    {
        await _context.Set<AcquisitionSourceTypeEntity>().AddAsync(entity, ct);
        return await _context.SaveChangesAsync(ct) > 0;
    }

    public async Task<bool> UpdateAsync(AcquisitionSourceTypeEntity entity, CancellationToken ct)
    {
        _context.Set<AcquisitionSourceTypeEntity>().Update(entity);
        return await _context.SaveChangesAsync(ct) > 0;
    }
}
