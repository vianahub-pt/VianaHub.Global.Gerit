...
{
    Task<IEnumerable<ClientConsentResponse>> GetAllAsync(int clientId, CancellationToken ct);
    Task<ClientConsentDetailResponse> GetByIdAsync(int clientId, int id, CancellationToken ct);
    Task<ListPageResponse<ClientConsentResponse>> GetPagedAsync(int clientId, PagedFilterRequest request, CancellationToken ct);
