...
namespace VianaHub.Global.Gerit.Api.Validators.Billing.Plan;
...
public class UpdatePlanRouteValidator : AbstractValidator<UpdatePlanRequest>
{
    ...
    public UpdatePlanRouteValidator(ILocalizationService localization)
    {
        ...
        When(x => x.MaxPhotosPerVisit > 0, () =>
        {
            RuleFor(x => x.MaxPhotosPerVisit)
                .GreaterThan(0).WithMessage(_localization.GetMessage("Api.Validator.Plan.Update.MaxPhotosPerVisit"));
        });
